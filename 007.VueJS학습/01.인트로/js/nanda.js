...
const vm = new Vue({
  ...
  // [5. vue 랜더링 완료단계 : mounted 랜더링 후 자동 실행 구역]
  mounted() {
    //=> 이 단계에서 DOM관련 스크립트 코드를 해준다
    //1. 제목 숨겼다 보이기
    $(".tit").hide().delay(1000).slideDown(800);
    // 2. 로고 왼쪽에서 날아오기
    $(".logo").css({translate:"-100vw"}).delay(2000).animate({
        translate: "0"
    },800,"easeOutBounce",
    //애니 후 실행구역
    ()=>{
        ...
        //-> 상품리스트 박스 위치로 이동 
        //$(상품리스트박스).offset().top
        $("html,body").animate({scrollTop: $(".gwrap").offset().top+"px"},600,"easeInOutQuart")
    });
  }, //mounted///////////////////////
}); /////// Vue ///////////////////
