...
class GoghWork extends React.Component {
  //클래스형 컴포넌트에서는 render()매서드로
  //html코드를 리턴함! render 내부에 return구문 필요
  render() {
    return (
      <React.Fragment>
        <h2>안녕 나는 고흐그림이양</h2>
...
/************************************************* 
    ...
    -> 중괄호를 쓰면 개별적인 속성변수를 전달받을 수 
        있다.(구조분해할당방식!)
*************************************************/

//내가 좋아하는 색 표시하기 컴포넌트///////////////////////
function FavoriteThings(쬬아) {
  return(
    <h2>
    내가 좋아하는 색은 {쬬아.color}이당
    <br />
    그리고 좋아하는 음식은 {쬬아.food}이당
    <br />
    취미는 {쬬아.hobby}야! 알긋냐?
  </h2>
  );
...
//컴포넌트를 호출하면 개별적으로 속성값을 구분할 수 있다
//출력: 3번째 .root

ReactDOM.render(<FavoriteThings color="pink" food="pizza, 족발먹구싶" hobby="game"/>, target[3])
