...
//4-1. 작가 타이틀과 그림 찍기 컴포넌트////////////
//구성: 작가이름 + 작품이미지
//데이터 : 작가이름(painter)
//         이미지 경로(작가이름의 객체 - workUrl)
//         작품명(Wname)
...
//4-2 전체출력 컴포넌트/////////////////
//구성 : 전체타이틀(setTitle컴포넌트) + 변경버튼 + 작가타이틀과 그림출력(MakePainting 컴포넌트)
//특이사항: 변경버튼 클릭시 MakePainting컴포넌트의 데이터를 변경하여 다시 출력하도록 함! (Hook사용!)

function ShowLikePainter({ isChange }) {
  //IsChange = true or false 값을 받아 기존작가를 변경하는 여부 전달변수
  //true = 바꾸겠다 / false = 기존값 유지
  ...
  return (
    <React.Fragment>
      ...
      style={{
        ...
        padding:"10px",
        fontWeight:"bold",
        backgroundColor:result?"pink":"skyblue",
        color:result?"blue":"hotpink",
    }}>작가변경!</button>
      {/* 3. 작품출력 */}
      {
        //삼항연산자로 isChange값에 따라 출력변경하기
        //isChange =변경=> result 상태관리 변수 사용
        result ? (
