...
function App() {
  ...
  //국적표시 객체
  //-> 매번 컴포넌트가 리렌더링 될떄마다 할당되는 객체의 주소값이 변경된다!
  //   const nara = {
  //     country: isKor ? "한국" : "일본",
  //   };

  //처음 할당된 객체의 주소값을 매모하여! 재사용하면
  //다른 상태변수가 변경될 때 같은 주소값을 유지하여 나라가 변경되지 않는다!
  //사용법: 변수 = Recat.useMemo(()=>{return객체},[의존성])
  ...
  //nara객체는 isKor에 의존하고있음
  //-> isKor이 변경되면 nara가 변경됨
  ...
  React.useEffect(() => {
    console.log("nara가 변경됨!", nara);
    //축구선수 이미지 중 해당 나라 이미지가 위로 올라옴
    $("img")
      .eq(isKor ? 1 : 0) //isKor true: 손흥민
      .animate({ bottom: "+=50px" }, 200);
    //buttom기존값에 50px씩 더해서 이동
  }, [nara]);
  ...
  ********************************************************/
  ...
  return (
    <header className="header" style={{ textAlign: "center" }}>
      ...
      <h1>
        <br /> {isKor ? "한국이" : "일본이"}몇 점차로 승리를 예측합니다
      </h1>
      {/* 점수입력창 */}
      <input
        type="number"
