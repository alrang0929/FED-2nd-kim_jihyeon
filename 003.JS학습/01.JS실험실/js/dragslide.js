...
function slideFn(selEl,slider) {
  ...
  //왼쪽으로 이동할 기준값 = 기준위치값 x 1.1
  let vlaFirst = leftVal * 1.1;
  //오른쪽으로 이동할 기준값 = 기준위치값 x 0.9
  let vlaSecond = leftVal * 0.9;
  console.log("left 셋팅값:", leftVal);
  console.log("left 기준값 110%:", vlaFirst);
  console.log("left 기준값의 90%:", vlaSecond);
  ...
  mFn.addEvt(dtg, "mouseup", (e) => {
    ...
    //대상슬라이드 이동 기준 준비하기
    //픽셀 단위를 parseInt로 지우기
    let currentLeft = parseInt(dtg.style.left);

    if(currentLeft < vlaFirst){
      console.log('왼쪽이동');
    }////if
    
    else if (currentLeft > vlaSecond){
      console.log('오른쪽 이동');
      
    }//else if
    else{ //valFirst 와 valSecond의 사위 범위
      console.log('제자리 이동');
    }
