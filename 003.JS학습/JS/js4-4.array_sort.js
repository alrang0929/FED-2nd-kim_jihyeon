      ...
      *************************************************
      ...
      2. 변경방법:
          ...
          (2) 변경확인 
              변경전 : {속성1:값1,속성2:값2}
              결과: 
              <값(=키)배열 = Object.keys(객체)>
              변경후 : [속성1,속성2]
              <값(=키)배열 = Object.values(객체)>
              변경후 : [값1,값2]
...
if(kWord == ""){
  alert("검색어를 입력해주세요!");
  //입력창에 포인터 들어가기! focuse()
  kWord.focus();
  return;
}
console.log(cta,kWord);

//4. 검색기준으로 검색어를 사용하여 검색한다
//검색대상 데이터 배열: list2
//사용 배열메서드: filter()
...
//searchResult?searchResult:list2
// => searchResult값 true -> searchResult값 전달
// => false -> 원본 list2 전달 
...
//5-2. 객체 데이터를 값배열로 변환 후 리스트 생성함수에 전달하여 리스트 화면출력
...
//list3 = 객체! 따라서 값배열로 변환한 배열(arrList3)을 보낸다
