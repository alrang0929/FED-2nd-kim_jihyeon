...
function changeSort(e, arrObj) {
  ...
  // arrObj 배열전달변수 => 원본배열을 담음! (주소복사가 됨 ..ㅠ)
  //원본배열 보존을 위해 딥카피를 함!
  //배열값=일반배열값 -> 스프레드 연상자를 사용하여 확장[...]
  // 1. 선택옵션값 읽어오기
  ...
  if (optVal == 1) {
    ...
    //해석: 앞값 뒷값이 같으면0, 뒷값이 크면 -1, 앞값이 크면 1
    //-> 즉, 앞값이 크면 자리를 바꿔서 유지! -> 오름차순 정렬

    //[공통정렬 처리하기]
    //문자든 숫자든 sort()의 내무적 처리에서
    //앞, 뒤 문자가 같으면 0
    //뒷 문자가 크면 -1, 뒷 문자가 작으면 1로 return값을
    //처리하면 된다.
    //-> [숫자 시그널]: 0 = 아무것도 안함 / 1 = 변경 후 유지
    //-> [내부적 처리란?]
    //문자일 경우 ['가'>'나'= 1]
    // -> [나,가]로 순서 변경되어야함(내림차순)
    //즉, 문자열도 순서대로 글자 알파벳, 가나다라순 등
    //특정 순서기준이 브라우저에 구현되어 있음!
  ...
  // 2-2. 내림차순 : 값 2
    ...
    arrObj.sort((a, b) => (a == b ? 0 : a < b ? 1 : -1));
    //해석: 앞값 뒷값이 같으면0, 뒷값이 크면 자리를 바꿔서 유지, 앞값이 크면 유지! <<내림차순
  ...
  // 원본배열확인하기
  console.log("정렬후 원본배열:", "sel" ? arrNumber : arrString);
} ////////// changeSort 함수 /////////////
